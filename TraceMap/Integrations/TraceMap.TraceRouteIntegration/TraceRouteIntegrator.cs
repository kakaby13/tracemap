...
namespace TraceMap.Integration.TraceRoute
{
    public class TraceRouteIntegrator : IIntegrator
    {
        ...
        {
            return targets.Select(target => new TraceInfo(target, $"traceroute {target}".Bash())).ToList();
        }

        private static List<Vertex> BuildBranch(TraceInfo response, Vertex rootNode)
        {
